...
class ClassicGameManager
{
    ...
    public function generate(): GameBoard
    {
        $seed = Str::random();
    ...
    public function getIslandCells(): array
    {
        $result = collect();
    ...
    private function fillWaterCells(GameBoard $result): void
    {
        for ($col = 0; $col < self::BOARD_COLS; $col++) {
            $result->setCell(0, $col, new Cell(CellType::Water, true));
            $result->setCell(self::BOARD_ROWS - 1, $col, new Cell(CellType::Water, true));
        }

        for ($row = 0; $row < self::BOARD_ROWS; $row++) {
            $result->setCell($row, 0, new Cell(CellType::Water, true));
            $result->setCell($row, self::BOARD_COLS - 1, new Cell(CellType::Water, true));
        }

        $result->setCell(1, 1, new Cell(CellType::Water, true));
        $result->setCell(1, self::BOARD_COLS - 2, new Cell(CellType::Water, true));
        $result->setCell(self::BOARD_ROWS - 2, 1, new Cell(CellType::Water, true));
        $result->setCell(self::BOARD_ROWS - 2, self::BOARD_COLS - 2, new Cell(CellType::Water, true));
    }
